...
public static class HttpHelper
{
    ...
    {
        var client = new HttpHelperWebClient(settings);
    ...
    public static TResultType JsonRequest<TResultType>(HttpRequestSettings settings)
    {
        ...
        if (settings.Content != null)
        {
            ...
            {
                settings.CapturedRequestContent = JsonSerializationHelper.Serialize(settings.Content,
        ...
        return (TResultType)JsonSerializationHelper.Deserialize(jsonResult, typeof(TResultType), true);
    }
    ...
    public static string DownloadImageToFile(string imageUrl, string filename = null, HttpRequestSettings settings = null)
    {
        ...
        {
            filename = Path.Combine(Path.GetTempPath(), "~img-" + DataHelper.GenerateUniqueId());
        }
        ...
        try
        {
            ...
            var ext = ImageHelper.GetExtensionFromMediaType(ct);
            if (ext == null)
...
/// <summary>
/// Configuration object for Http Requests used by the HttpHelper
