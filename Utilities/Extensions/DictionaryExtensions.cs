...
public static class DictionaryExtensions
{
    ...
    public static string ToXml(this IDictionary items, string root = "root")
    {
        ...
        {
            string xmlType = XmlHelper.MapTypeToXmlType(item.Value.GetType());
            ...
            else
            {
                ...
                if (SerializationHelper.SerializeObject(item.Value, out xmlString))
                {
    ...
    public static void FromXml(this IDictionary items, string xml)
    {
        ...
        foreach (XElement el in root.Elements())
        {
            ...
            if (!string.IsNullOrEmpty(typeString) && typeString != "string" && !typeString.StartsWith("__"))
            {
                ...
                Type type = XmlHelper.MapXmlTypeToType(typeString);
                if (type != null)
                    items.Add(el.Name.LocalName, ReflectionHelper.StringToTypedValue(val, type));
                else
                    items.Add(el.Name.LocalName, val);
            }
            else if (typeString.StartsWith("___"))
            {
                Type type = ReflectionHelper.GetTypeFromName(typeString.Substring(3));
                object serializationUtilsDeSerializeObject = SerializationHelper.DeSerializeObject(el.Elements().First().CreateReader(), type);
